...
<?php
$editID = $_GET['proses_edit_id'];
$sql = "SELECT * FROM processes where PROCESS_CODE='$editID'";
$query = mysqli_query($conn, $sql);

if ($query) {
  foreach ($query as $row) {
    if ($editID == $row['PROCESS_CODE']) {
      $tempPC = $row['PROCESS_CODE'];
      $tempPN = $row['PROCESS_NAME'];
      $tempMC = $row['MODULE_CODE'];
      $tempPS = $row['PROCESS_STATUS'];
    }
  }
} else {
  echo '<script>swal({
...
<head>
  ...
  <title>Kemas Kini Proses | PS</title>
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <div class="row">
              <div class="col mb-4 order-0">
                ...
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Proses /</span> Maklumat Proses</h4>
                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h5 class="mb-0">Maklumat Proses</h5>
                      <small class="text-muted float-end"></small>
                    </div>
                    <div class="card-body">
                      <form method="POST" action="" name="editFormProses">
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-fullname">Kod Proses</label>
                        ...
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-email">Nama Modul</label>
                          <div class="input-group input-group-merge">
                            <span class="input-group-text"><i class="bx bxs-window-alt"></i></span>
                            <select id="edMKod" name="edMKod" class="form-select" required>
                              <?php
                        ...
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-fullname">Nama Proses</label>
                          <div class="input-group input-group-merge">
                            <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bx-window-alt"></i></span>
                            <input style="text-transform:uppercase" type="text" class="form-control" id="edPName" name="edPName" placeholder="CONTOH : DAFTAR KONTRAK BARU" value="<?= $tempPN ?>" aria-label="" aria-describedby="basic-icon-default-fullname2" required/>
                          </div>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-email">Status Proses</label>
                          <div class="input-group input-group-merge">
                            <span class="input-group-text"><i class="bx bxs-quote-single-left"></i></span>
                            <select class="form-select" id="edPStatus" name="edPStatus" aria-label="Default select example">
                              <?php if ($tempPS == 0) {
                        ...
                        <div align="right">
                          <button type="button" id="bProses" name="editProses" class="btn btn-outline-secondary" onclick="location.href='../../pentadbir_sistem/proses/proses.php'">
                            <span class="tf-icons bx bx-x"></span>&nbsp; Batal
                          </button>
                          <button type="submit" id="editProses" name="editProses" class="btn btn-primary">
                            <span class="tf-icons bx bx-save"></span>&nbsp; Simpan
                          </button>
                        </div>
                      </form>
                      <?php
                      if (isset($_POST['editProses'])) {
                        ...
                        $sql = "UPDATE processes SET PROCESS_NAME='$ed_PN',PROCESS_STATUS='$ed_PS',PROCESS_DATE='$ed_PD',MODULE_CODE='$ed_MC' WHERE PROCESS_CODE='$editID'";
                        ...
                        if ($query) {
                          echo '<script>swal({
                          title: "Berjaya!",
                          text: "Maklumat proses berjaya dikemaskini!",
                          type: "success",
                          icon: "success",
                        }).then(function() {
                          document.location ="../../pentadbir_sistem/proses/proses.php";
                        });
                        </script>';
                        } else {
                          echo '<script>swal({
