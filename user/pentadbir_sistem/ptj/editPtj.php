...
<!-- db connection -->
<?php
$editID = $_GET['ptj_edit_id'];
$sql = "SELECT * FROM ptj where PTJ_CODE='$editID'";
$query = mysqli_query($conn, $sql);

if ($query) {
  foreach ($query as $row) {
    if ($editID == $row['PTJ_CODE']) {
      $tempPC = $row['PTJ_CODE'];
      $tempPN = $row['PTJ_NAME'];
      $tempPPre = $row['PTJ_PREFIX'];
      $tempPA = $row['PTJ_ADDRESS'];
      $tempPS = $row['PTJ_STATUS'];
      $tempPP = $row['PTJ_PHONE'];
    }
  }
} else {
  echo '<script>swal({
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <div class="row">
              <div class="col mb-4 order-0">
                ...
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">PTJ /</span> Maklumat PTJ</h4>
                ...
                <div class="col-xl">
                  <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                      <h5 class="mb-0">Ubah Maklumat PTJ</h5>
                      <small class="text-muted float-end"></small>
                    </div>
                    <div class="card-body">
                      <form method="POST" action="" name="editFormPTJ">
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-fullname">Kod PTJ</label>
                          <div class="input-group input-group-merge">
                            <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bx-code"></i></span>
                            <input type="text" class="form-control" id="edPKod" name="edPKod" value="  <?php echo $tempPC ?>" aria-label="" aria-describedby="basic-icon-default-fullname2" readonly />
                          </div>
                        </div>
                        <div class="mb-3">
                          ...
                          <div class="input-group input-group-merge">
                            ...
                            <input style="text-transform:uppercase" type="text" class="form-control" id="edPName" name="edPName" placeholder="CONTOH : JABATAN AKAUNTAN NEGERI SELANGOR" value="<?php echo $tempPN ?>" aria-label="Perancangan Perolehan Tahunan" aria-describedby="basic-icon-default-fullname2" required/>
                          </div>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-fullname">Singkatan</label>
                          <div class="input-group input-group-merge">
                            <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bxs-buildings"></i></span>
                            <input style="text-transform:uppercase" type="text" class="form-control" id="edPPre" name="edPPre" placeholder="CONTOH : JANS" value="<?php echo $tempPPre ?>" aria-label="Perancangan Perolehan Tahunan" aria-describedby="basic-icon-default-fullname2" required/>
                          </div>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-company">No. Telefon</label>
                          <div class="input-group input-group-merge">
                            <span id="basic-icon-default-company2" class="input-group-text"><i class="bx bxs-phone-call"></i></span>
                            <input type="tel" id="edPPhone" name="edPPhone" class="form-control" placeholder="CONTOH : 096958205" aria-label="096958205" value="<?php echo $tempPP ?>" aria-describedby="basic-icon-default-company2" required/>
                          </div>
                        </div>
                        <div class="mb-3">
                          <label class="form-label" for="basic-icon-default-company">Alamat</label>
                          <div class="input-group input-group-merge">
                            <span id="basic-icon-default-company2" class="input-group-text"><i class="bx bx-current-location"></i></span>
                            <input style="text-transform:uppercase" type="text" id="edPAddr" name="edPAddr" class="form-control" placeholder="CONTOH : LOT 2956,JALAN IKM,..." aria-label="lot 2956,jalan IKM,..." value="<?php echo $tempPA ?>" aria-describedby="basic-icon-default-company2" required/>
                          </div>
                        </div>
                        <div class="mb-3">
                        ...
                        <div align="right">
                          <button type="button" id="bPTJ" name="bPTJ" class="btn btn-outline-secondary" onclick="location.href='../../pentadbir_sistem/ptj/ptj.php'">
                            <span class="tf-icons bx bx-x"></span>&nbsp; Batal
                          </button>
                          <button type="submit" id="editPTJ" name="editPTJ" class="btn btn-primary">
                            <span class="tf-icons bx bx-save"></span>&nbsp; Simpan
                          </button>
                        </div>
                      </form>
                      <?php
                      if (isset($_POST['editPTJ'])) {

                        date_default_timezone_set('Asia/Kuala_Lumpur');
                        $date = date('h:i  d-m-y');

                        $ed_PN = strtoupper($_POST['edPName']);
                        $ed_PP = $_POST['edPPhone'];
                        $ed_PPre = strtoupper($_POST['edPPre']);
                        $ed_PS = $_POST['edPStatus'];
                        $ed_PA = strtoupper($_POST['edPAddr']);
                        $ed_PD = $date;

                        $sql = "UPDATE ptj SET PTJ_NAME='$ed_PN', PTJ_PHONE='$ed_PP', PTJ_ADDRESS='$ed_PA', PTJ_STATUS='$ed_PS', PTJ_DATE='$ed_PD',PTJ_PREFIX='$ed_PPre' WHERE PTJ_CODE='$editID'";
                        ...
                        if ($query) {
                          echo '<script>swal({
                          title: "Berjaya!",
                          text: "Maklumat PTJ berjaya dikemaskini!",
                          type: "success",
                          icon: "success",
                        }).then(function() {
                          document.location ="../../pentadbir_sistem/ptj/ptj.php";
                        });
                        </script>';
                        } else {
                          echo '<script>swal({
