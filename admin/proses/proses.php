  ...
  <script>
    function toExcel() {
      $("#tableD").table2excel({
        ...
        name: "Worksheet Name",
        filename: "Senarai Proses.xls", // do include extension
    ...
    function toPDF() {
      html2canvas($('#tableD')[0], {
        onrendered: function(canvas) {
          ...
          pdfMake.createPdf(docDefinition).download("Senarai Proses.pdf");
        }
      });
    }
  </script>
    ...
    <div class="layout-container">
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <!-- Modal -->
          <div class="modal fade" id="addProsesModal" data-bs-backdrop="static" tabindex="-1">
            <div class="modal-dialog">
              <form class="modal-content" name="formProses" id="formProses" action="" method="POST">
                <div class="modal-header">
                  <h5 class="modal-title">Tambah Proses</h5>
                ...
                <div class="modal-body">
                  <div class="row">
                    <div class="col mb-3">
                      <label for="defaultSelect" class="form-label">Nama Modul</label>
                      <select id="inModulKod" name="inModulKod" class="form-select" required>
                  ...
                  <div class="row">
                    <div class="col mb-3">
                      <label for="inPName" class="form-label">Nama Proses</label>
                      <input style="text-transform:uppercase" type="text" id="inPName" name="inPName" class="form-control" placeholder="CONTOH : TAMBAH PENGGUNA" required />
                    </div>
                  </div>
                </div>
                <div class="modal-footer">
                  ...
                  <button type="submit" class="btn btn-primary" name="addProses">Tambah</button>
                </div>
              </form>
              <?php
              if (isset($_POST['addProses'])) {
                date_default_timezone_set('Asia/Kuala_Lumpur');
                $date = date('h:i  d-m-y');

                $modulC = $_POST['inModulKod'];
                $prosesName = strtoupper($_POST['inPName']);
                $prosesD = $date;

                $isProExist = false;
                $sqlP = "SELECT * FROM procesess";
                ...
                if (!$queryP) {
                  ...
                  foreach ($queryP as $row) {
                    if ($row['MODULE_CODE'] == $modulC || $row['PROCESSES_NAME'] == $prosesD) {
                      $isProExist = true;
                      break;
                    }
                  }
                ...
                if ($isProExist) {
                  ...
                  $sql = "INSERT INTO processes (MODULE_CODE,PROCESS_NAME,PROCESS_STATUS,PROCESS_DATE) VALUES ('$modulC','$prosesName',1,'$prosesD') ";
                  ...
                  if ($query) {
                    echo '<script>swal({
                    title: "Berjaya!",
                    text: "Maklumat Proses berjaya ditambah",
                    type: "success",
                    icon: "success",
                    });
                    </script>';
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <div class="row">
              <div class="col mb-4 order-0">
                <!-- Striped Rows -->
                <h4 class="fw-bold py-3 mb-4">Proses</h4>

                <div class="card">

                  <h5 class="card-header d-flex justify-content-between align-items-center">Senarai Proses
                    <div class="demo-inline-spacing">
                      ...
                      <button data-bs-toggle="modal" data-bs-target="#addProsesModal" type="button" class="btn  btn-primary">
                        <span class='bx bxs-add-to-queue'></span>&nbsp; Tambah Proses
                      </button>
                    </div>
                  </h5>
                  <br>
                        ...
                        function showData($conn)
                        {
                          ...
                          if (!$query) {
                            ...
                            foreach ($query as $row) {
                              ...
                              $tempp = $row['MODULE_CODE'];
                              $sql2 = "SELECT * FROM modules WHERE MODULE_CODE=$tempp";
                              $query2 = mysqli_query($conn, $sql2);
                              $tempMP = "";
                              $tempMN = "";
                              foreach ($query2 as $row2) {
                                $tempMN = $row2['MODULE_NAME'];
                                $tempMP = $row2['MODULE_PREFIX'];
                              }
                                ...
                                <td>
                                  <div class='dropdown'>
                                    ...
                                    <div class='dropdown-menu'>
                                      <a class='dropdown-item' href='editProses.php?proses_edit_id={$row['PROCESS_CODE']}'><i class='bx bx-edit-alt me-1'></i> Ubah</a>
                                      
                                      <a class='dropdown-item' href='#' onclick='updateStatus({$row['PROCESS_CODE']});'><i class='bx bx-check'></i>  Aktif</a>
                                    </div>
                                  </div>
                                </td>
																</tr>\n";
                                ...
                                <td>
                                  <div class='dropdown'>
                                    ...
                                    <div class='dropdown-menu'>
                                      ...
                                      <a class='dropdown-item' href='#' onclick='updateStatus({$row['PROCESS_CODE']});'><i class='bx bx-x'></i>  Tidak Aktif</a>
                                    </div>
                                  </div>
                                </td>
																</tr>\n";
                              ...
                              // echo " <a onclick='return confirmDelete(this);' class='dropdown-item' href='deleteProses.php?proses_delete_id={$row['PROCESS_CODE']}'><i class='bx bx-trash me-1'></i> Delete</a>";
                            }
                          }
                          if ($i == 0) {
